...
function Card ({
  ...
  return (
    <div className='w-full dark:border-gray-700 border border-gray-200 dark:border-none rounded-lg transition-all duration-500'>
      <div className={`bg-card-accent dark:bg-cardD-accent hover:bg-card-Hover hover:dark:bg-cardD-Hover shadow ${className} transition-all duration-500`}>
        { children }
      </div>
    </div>
  )
...
Card.Header = function CardHeader ({ children, ...props }: Props): React.ReactElement {
  ...
  if (typeof children === 'string') {
    component = (
    <h5 className="text-2xl font-bold tracking-tight text-card-content dark:text-cardD-content">
      { children }
    </h5>)
  }
  return (
